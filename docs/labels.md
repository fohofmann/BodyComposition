...
- `labels` are the unprocessed and unchanged output of the respective segmentation model. Thus, they differ between models and are not directly comparable. For instance, for body composition `labels` refer to compartments (e.g., muscle compartment, visceral compartment, subcutaneous compartment, etc.) as identified by the respective model. The canonical internal pipeline persists this map even when the fast preset otherwise avoids intermediate labels.
...
The executable defaults, LAMA/NAMA and IMAT distinction, alternative
literature windows, and cleanup profiles are specified in
