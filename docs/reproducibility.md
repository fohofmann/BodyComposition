...
3. Create one ordered `batch_input.schema.json` manifest. A directory produced
   by `bodycomposition convert DICOM_ROOT OUTPUT_DIRECTORY` includes a safe
   starting manifest; review and freeze it for the governed run. Record
   `series_uid` explicitly when one manifest case points at a DICOM directory
   containing multiple CT series.
...
Automatic directory analysis is convenient for ordinary processing, but do
not use file names, directory order, or a changing directory/database query as
the implicit definition of a governed research cohort.
