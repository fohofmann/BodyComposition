"""Connected-component and morphology helpers for label masks."""
...
def _connectivity_rank(ndim: int, connectivity: int) -> int:
    """Translate conventional neighbor counts to scikit-image ranks."""
    ...
    try:
        ...
        raise ValueError(
            f"Connectivity for a {ndim}D mask must be one of {allowed}, got {connectivity}."
        ) from error
...
def _component_size_threshold(
    ...
    *,
    ...
    """Convert a physical or voxel threshold to a component voxel count."""
...
def _remove_small_objects_2d(mask, min_size=10, connectivity=8):
    """Return a 2-D mask without components below ``min_size`` pixels."""
...
def _remove_small_objects_3d(
    ...
    """Return a 3-D mask after component-size and extent filtering."""
    ...
    labels_filtered = [
        prop.label for prop in props if prop.area >= float(min_size) and prop.extent >= min_extent
    ]
    return np.isin(labels, labels_filtered)


def filter_hu(image_np: np.ndarray, hu_range: list):
    """Return voxels inside an inclusive HU interval."""
...
def remove_small_objects(
    ...
    *,
    ...
    if (
        spacing_xyz.shape != (3,)
        or not np.all(np.isfinite(spacing_xyz))
        or np.any(spacing_xyz <= 0)
    ):
        raise ValueError(
            f"spacing_xyz must contain three finite positive values, got {spacing_xyz}."
        )
    ...
    if limit_size_version == "2D" and limit_size_2D > 0:
        ...
        mask_np[:] = _remove_small_objects_3d(
            mask_np,
            min_size=threshold,
            min_extent=0,
            connectivity=connectivity,
...
def _fill_small_holes_nd(mask: np.ndarray, maximum_size: float, connectivity: int) -> np.ndarray:
    ...
    border = np.concatenate(
        [labels.take(0, axis=axis).ravel() for axis in range(boolean.ndim)]
        + [labels.take(-1, axis=axis).ravel() for axis in range(boolean.ndim)]
    )
...
def filter_keep_largest(mask_np, labels=None):
    """Keep the largest connected component of each selected label in-place."""
...
def fill_holes(mask_np, labels=None):
    """Fill enclosed holes for each label while preserving exterior background.

    A one-voxel background frame makes the exterior component unambiguous when
    a labeled object touches an image boundary.
    """
