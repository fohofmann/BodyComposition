    ...
    def __call__(self, memory):
        ...
        if output_label.exists() and self.config['run']['skip']:
            ...
            with contextlib.redirect_stdout(sl), contextlib.redirect_stderr(sl):
                ...
                output_label.data = self.predictor.predict_single_npy_array(input_image.data[None], 
...
# action class
class SegmIntBodyComposition(PipelineAction):
    """
    Spine Segmentation using nnUNet
    Ref: https://huggingface.co/collections/fhofmann/
    """
    ...
    def __init__(self, pipeline, image: str, model: str = 'ResEncM'):
        ...
        if model == 'ResEncM':
            logging.info('  using fast model: ResEncM, fold=all')
            model_path = self.config['paths']['weights']['int-bodycomposition'] + '/nnUNetTrainer__nnUNetResEncUNetMPlans__3d_fullres'
            model_folds = 'all'
        elif model == 'ResEncL':
            logging.info('  using standard model: ResEncL, fold=[0,1,2,3,4]')
            model_path = self.config['paths']['weights']['int-bodycomposition'] + '/nnUNetTrainer__nnUNetResEncUNetLPlans__3d_fullres'
            model_folds = [0, 1, 2, 3, 4]
        else:
            raise ValueError(f'unknown model: {model}')
